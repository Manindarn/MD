								...
								<?php
									...
									foreach($mongoDBs as $database){
										...
										if(!in_array($database->getName(), $hiddenDBsArray)){
											echo '<li><input type="checkbox" name="mongoSourceList[]" value="'.$database->getName().'"> '.$database->getName();
											...
											echo '<ul class="ul-sub bg-light-gray">';
												foreach ($thisDBCollections as $collection) { 
													echo '<li><input type="checkbox" name="mongoSourceList[]" value="'.$database->getName().'"> '.$collection->getName().'</li>';
												}

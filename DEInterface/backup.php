...
class DEInterface {
    ...
    public function getOrgStudents($orgIds = []){
     ...
     public function getMongoInput($Postdata) 
        {
          // print_r($Postdata);die();
           // foreach($Postdata as $orgID)
            //{
            //$dbAndCollectionArray = explode('Verticle', $orgID);
            //print_r($dbAndCollectionArray);die();
            //$MongoDBOBJ = new MongoDB($dbAndCollectionArray[1], $dbAndCollectionArray[0]);
          //$test = $MongoDBOBJ->find(array("orgID"=> array('$exists' => true)));
           // print_r($test);

            //$this->writeJsonResponce($MongoDBOBJ);
            $test = new MongoDB('MailLog');
                      
            /*$res = $cursor->find(array("mailContent"=> array('$exists' => true)));
            ...
            //echo"hellooutput"; echo $res;
           // }
                         // retrieve existing document
                $trackname = "hgssajhdg";
                  $criteria = array('mailContent' => $trackname 
                  );
                  $doc = $test->findOne($criteria);
                  if(!empty($doc)) {
        ...
        public function testf($Postdata){
           // print_r($Postdata);die();
                    //$this->data = $Postdata;

                 $test = new MongoDB('Organizations');

                /* $trackname = "drwerwe";
                  $criteria = array('mailContent' => $trackname 
                  );
                  $doc = $test->find($criteria);
                  if(!empty($doc)) {
                     echo 'Data Already Exist';
                  } 
                  else {
                ...
                $user = $test->find(array("orgID" => $Postdata));
                print_r($user);
                if ($user  > 0)
                    //return $user;
                    echo "pass";
